...
@NoArgsConstructor
@AllArgsConstructor
@ApiModel(value = "晚班运动区秩序照片表参数", description = "")
public class NightSportAreaImagesParam implements Serializable {
    /**
     * ID系统自动生成
     */
     ...
     */
    @ApiModelProperty(value = "晚班运动区秩序ID day_night_sport_area.id")
     ...
     * 将查询参数转化为查询Wrapper
     */
    public QueryWrapper<NightSportAreaImages> toQueryWrapper() {
        QueryWrapper<NightSportAreaImages> wrapper = Wrappers.<NightSportAreaImages>query();
        wrapper.eq(this.getId() != null, "id", this.getId());
        wrapper.in(this.getIdList() != null && this.getIdList().size() > 0, "id", this.getIdList());
        wrapper.eq(this.getSportAreaId() != null, "sport_area_id", this.getSportAreaId());
        if (this.getUrl() != null) {
