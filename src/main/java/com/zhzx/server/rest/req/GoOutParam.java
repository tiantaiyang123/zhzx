...
@NoArgsConstructor
@AllArgsConstructor
@ApiModel(value = "走读生提前出门表参数", description = "")
public class GoOutParam implements Serializable {
    /**
     * ID系统自动生成
     */
     ...
     * 走读生提前出门情况
     */
    @ApiModelProperty(value = "走读生提前出门情况")
    private DayOrderEnum goOutOrder;
    /**
     * 走读生提前出门情况 IN值List
     */
    @ApiModelProperty(value = "走读生提前出门情况 IN值List")
    private List<String> goOutOrderList;
    /**
     * 开始时间
     */
     ...
     * 将查询参数转化为查询Wrapper
     */
    public QueryWrapper<GoOut> toQueryWrapper() {
        QueryWrapper<GoOut> wrapper = Wrappers.<GoOut>query();
        wrapper.eq(this.getId() != null, "id", this.getId());
        wrapper.in(this.getIdList() != null && this.getIdList().size() > 0, "id", this.getIdList());
        wrapper.eq(this.getLeaderDutyId() != null, "leader_duty_id", this.getLeaderDutyId());
        wrapper.eq(this.getGoOutOrder() != null, "go_out_order", this.getGoOutOrder());
        wrapper.in(this.getGoOutOrderList() != null && this.getGoOutOrderList().size() > 0, "go_out_order", this.getGoOutOrderList());
