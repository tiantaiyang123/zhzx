...
@NoArgsConstructor
@AllArgsConstructor
@ApiModel(value = "晚餐情况照片表参数", description = "")
public class DinnerImagesParam implements Serializable {
    /**
     * ID系统自动生成
     */
     ...
     */
    @ApiModelProperty(value = "晚餐情况ID day_dinner.id")
     ...
     * 将查询参数转化为查询Wrapper
     */
    public QueryWrapper<DinnerImages> toQueryWrapper() {
        QueryWrapper<DinnerImages> wrapper = Wrappers.<DinnerImages>query();
        wrapper.eq(this.getId() != null, "id", this.getId());
        wrapper.in(this.getIdList() != null && this.getIdList().size() > 0, "id", this.getIdList());
        wrapper.eq(this.getDinnerId() != null, "dinner_id", this.getDinnerId());
        if (this.getUrl() != null) {
