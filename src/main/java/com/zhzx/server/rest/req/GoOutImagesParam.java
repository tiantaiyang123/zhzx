...
@NoArgsConstructor
@AllArgsConstructor
@ApiModel(value = "走读生提前出门照片表参数", description = "")
public class GoOutImagesParam implements Serializable {
    /**
     * ID系统自动生成
     */
     ...
     */
    @ApiModelProperty(value = "走读生提前出门ID day_go_out.id")
     ...
     * 将查询参数转化为查询Wrapper
     */
    public QueryWrapper<GoOutImages> toQueryWrapper() {
        QueryWrapper<GoOutImages> wrapper = Wrappers.<GoOutImages>query();
        wrapper.eq(this.getId() != null, "id", this.getId());
        wrapper.in(this.getIdList() != null && this.getIdList().size() > 0, "id", this.getIdList());
        wrapper.eq(this.getGoOutId() != null, "go_out_id", this.getGoOutId());
        if (this.getUrl() != null) {
