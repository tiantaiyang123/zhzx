...
@NoArgsConstructor
@AllArgsConstructor
@ApiModel(value = "晚餐情况表参数", description = "")
public class DinnerParam implements Serializable {
    /**
     * ID系统自动生成
     */
    ...
    private DayOrderEnum dinnerOrder;
    /**
     * 就餐秩序 IN值List
     */
    @ApiModelProperty(value = "就餐秩序 IN值List")
    private List<String> dinnerOrderList;
    /**
     * 是否陪餐
     */
     ...
     * 将查询参数转化为查询Wrapper
     */
    public QueryWrapper<Dinner> toQueryWrapper() {
        QueryWrapper<Dinner> wrapper = Wrappers.<Dinner>query();
        wrapper.eq(this.getId() != null, "id", this.getId());
        wrapper.in(this.getIdList() != null && this.getIdList().size() > 0, "id", this.getIdList());
        wrapper.eq(this.getLeaderDutyId() != null, "leader_duty_id", this.getLeaderDutyId());
        wrapper.eq(this.getDinnerOrder() != null, "dinner_order", this.getDinnerOrder());
        wrapper.in(this.getDinnerOrderList() != null && this.getDinnerOrderList().size() > 0, "dinner_order", this.getDinnerOrderList());
