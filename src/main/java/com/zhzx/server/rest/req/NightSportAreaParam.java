...
@NoArgsConstructor
@AllArgsConstructor
@ApiModel(value = "晚班运动区秩序表参数", description = "")
public class NightSportAreaParam implements Serializable {
    /**
     * ID系统自动生成
     */
     ...
     * 运动区秩序
     */
    @ApiModelProperty(value = "运动区秩序")
    private DayOrderEnum sportAreaOrder;
    /**
     * 运动区秩序 IN值List
     */
    @ApiModelProperty(value = "运动区秩序 IN值List")
    private List<String> sportAreaOrderList;
    /**
     * 开始时间
     */
     ...
     * 将查询参数转化为查询Wrapper
     */
    public QueryWrapper<NightSportArea> toQueryWrapper() {
        QueryWrapper<NightSportArea> wrapper = Wrappers.<NightSportArea>query();
        wrapper.eq(this.getId() != null, "id", this.getId());
        wrapper.in(this.getIdList() != null && this.getIdList().size() > 0, "id", this.getIdList());
        wrapper.eq(this.getLeaderDutyId() != null, "leader_duty_id", this.getLeaderDutyId());
        wrapper.eq(this.getSportAreaOrder() != null, "sport_area_order", this.getSportAreaOrder());
        wrapper.in(this.getSportAreaOrderList() != null && this.getSportAreaOrderList().size() > 0, "sport_area_order", this.getSportAreaOrderList());
