...
import com.zhzx.server.repository.NoonSportAreaImagesMapper;
import com.zhzx.server.repository.NoonSportAreaMapper;
import com.zhzx.server.repository.base.NoonSportAreaBaseMapper;
...
@Service
public class NoonSportAreaServiceImpl extends ServiceImpl<NoonSportAreaMapper, NoonSportArea> implements NoonSportAreaService {
    @Resource
    private NoonSportAreaImagesMapper noonSportAreaImagesMapper;
    @Resource
    private CommentService commentService;
    @Resource
    private IncidentMapper incidentMapper;
    @Override
    public int updateAllFieldsById(NoonSportArea entity) {
    ...
    @Override
    public boolean saveBatch(Collection<NoonSportArea> entityList, int batchSize) {
        String sqlStatement = SqlHelper.getSqlStatement(NoonSportAreaBaseMapper.class, SqlMethod.INSERT_ONE);
        return executeBatch(entityList, batchSize, (sqlSession, entity) -> sqlSession.insert(sqlStatement, entity));
    }

    @Override
    public NoonSportArea createOrUpdate(NoonSportArea entity) {
        ...
        if(CollectionUtils.isNotEmpty(entity.getIncidentList())){
            this.incidentMapper.delete(Wrappers.<Incident>lambdaQuery()
                    .eq(Incident::getClassify,"DAY_NOON_SPORT_AREA")
                    .eq(Incident::getDailyRoutineId,entity.getId())
            );
            entity.getIncidentList().stream().forEach(incident -> {
                incident.setClassify(ClassifyEnum.DAY_NOON_SPORT_AREA);
                incident.setDailyRoutineId(entity.getId());
                incident.setItemName(ClassifyEnum.DAY_NOON_SPORT_AREA.toString());
        ...
        if(CollectionUtils.isNotEmpty(entity.getCommentList())){
            ...
            List<Comment> commentList =  commentService.list(Wrappers.<Comment>lambdaQuery()
                    .eq(Comment::getClassify,"DAY_NOON_SPORT_AREA")
            ...
            entity.getCommentList().stream().forEach(commentDto -> {
                ...
                if(commentDto.getId() == null || commentDto.getState().equals(CommentStateEnum.TO_BE_PUSHED)){
                    commentDto.setClassify(ClassifyEnum.DAY_NOON_SPORT_AREA);
                    commentDto.setDailyRoutineId(entity.getId());
                    commentDto.setItemName(ClassifyEnum.DAY_NOON_SPORT_AREA.toString());
        ...
        noonSportAreaImagesMapper.delete(Wrappers.<NoonSportAreaImages>lambdaQuery()
                .eq(NoonSportAreaImages::getSportAreaId,entity.getId())
        );
        if(CollectionUtils.isNotEmpty(entity.getNoonSportAreaImagesList())){

            entity.getNoonSportAreaImagesList().stream().forEach(southGateImages -> {
                southGateImages.setSportAreaId(entity.getId());
                southGateImages.setDefault().validate(true);
            });
            noonSportAreaImagesMapper.batchInsert(entity.getNoonSportAreaImagesList());
        }
        return this.getById(entity.getId());
    }

    @Override
    public IPage pageDetail(IPage page, QueryWrapper<NoonSportArea> wrapper) {
        List<NoonSportArea> southGates = this.baseMapper.pageDetail(page,wrapper);
