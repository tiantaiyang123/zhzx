...
@NoArgsConstructor
@AllArgsConstructor
@TableName("`day_go_out`")
@ApiModel(value = "GoOut", description = "走读生提前出门表")
public class GoOut extends BaseDomain {
    /**
     * ID系统自动生成
     */
    ...
    @TableField(exist = false)
    private List<GoOutImages> goOutImagesList;
    /**
     ...
     * 走读生提前出门情况
     */
    @TableField(value = "go_out_order")
    @ApiModelProperty(value = "走读生提前出门情况", required = true)
     ...
     * 设置默认值
     */
    public GoOut setDefault() {
        if (this.getGoOutOrder() == null) {
            this.setGoOutOrder(DayOrderEnum.NORMAL);
        }
    ...
    public Boolean validate(Boolean throwException) {
        ...
        if (this.getGoOutOrder() == null) {
            if (throwException) throw new ApiCode.ApiException(-1, "走读生提前出门情况不能为空！");
            return false;
        }
