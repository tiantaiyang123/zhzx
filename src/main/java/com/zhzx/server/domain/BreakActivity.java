...
@NoArgsConstructor
@AllArgsConstructor
@TableName("`day_break_activity`")
@ApiModel(value = "BreakActivity", description = "大课间活动情况表")
public class BreakActivity extends BaseDomain {
    /**
     * ID系统自动生成
     */
    ...
    @TableField(exist = false)
    private List<BreakActivityImages> breakActivityImagesList;
    /**
     ...
     * 活动年级 多选 用[,]分割
     */
    @TableField(value = "activity_grade")
    @ApiModelProperty(value = "活动年级 多选 用[,]分割", required = true)
    ...
    @ApiModelProperty(value = "活动秩序", required = true)
     ...
     * 设置默认值
     */
    public BreakActivity setDefault() {
        if (this.getActivityOrder() == null) {
            this.setActivityOrder(DayOrderEnum.NORMAL);
        }
    ...
    public Boolean validate(Boolean throwException) {
        ...
        if (this.getActivityGrade() == null) {
            if (throwException) throw new ApiCode.ApiException(-1, "活动年级 多选 用[,]分割不能为空！");
            return false;
        }
        if (this.getActivityType() == null) {
            if (throwException) throw new ApiCode.ApiException(-1, "活动类型不能为空！");
            return false;
        }
        if (this.getActivityOrder() == null) {
            if (throwException) throw new ApiCode.ApiException(-1, "活动秩序不能为空！");
            return false;
        }
