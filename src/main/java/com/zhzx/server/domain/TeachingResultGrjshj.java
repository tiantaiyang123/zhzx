...
@NoArgsConstructor
@AllArgsConstructor
@TableName("`tmg_teaching_result_grjshj`")
@ApiModel(value = "TeachingResultGrjshj", description = "个人竞赛获奖表")
public class TeachingResultGrjshj extends BaseDomain {
    /**
     * ID系统自动生成
     */
    ...
    @ApiModelProperty(value = "比赛名称 和tmg_teaching_result.name一致 可以从sys_label.classify == GRJS_BSMC选择，也可以录入", required = true)
    private String bsmc;

    /**
     * 设置默认值
     */
    public TeachingResultGrjshj setDefault() {
        return this;
    }
    ...
    public Boolean validate(Boolean throwException) {
        ...
        if (this.getBsmc() == null) {
            if (throwException) throw new ApiCode.ApiException(-1, "比赛名称 和tmg_teaching_result.name一致 可以从sys_label.classify == GRJS_BSMC选择，也可以录入不能为空！");
            return false;
        }
        return true;
    }
