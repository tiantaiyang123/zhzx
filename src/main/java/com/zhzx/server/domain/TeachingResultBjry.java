...
@NoArgsConstructor
@AllArgsConstructor
@TableName("`tmg_teaching_result_bjry`")
@ApiModel(value = "TeachingResultBjry", description = "班级荣誉表")
public class TeachingResultBjry extends BaseDomain {
    /**
     * ID系统自动生成
     */
    ...
    @ApiModelProperty(value = "荣誉称号 和tmg_teaching_result.name一致", required = true)
    private String rych;
    /**
     * 获奖名称
     */
    @TableField(value = "hjmc")
    @ApiModelProperty(value = "获奖名称", required = true)
    private String hjmc;
    /**
     * 班级ID sys_clazz.id
     */
     ...
     * 设置默认值
     */
    public TeachingResultBjry setDefault() {
        return this;
    }
    ...
    public Boolean validate(Boolean throwException) {
        ...
        if (this.getRych() == null) {
            if (throwException) throw new ApiCode.ApiException(-1, "荣誉称号 和tmg_teaching_result.name一致不能为空！");
            return false;
        }
        if (this.getHjmc() == null) {
            if (throwException) throw new ApiCode.ApiException(-1, "获奖名称不能为空！");
            return false;
        }
